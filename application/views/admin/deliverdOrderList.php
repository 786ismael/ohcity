...
<?php include 'include/header.php';
 ...
 $arr_get = ['status'=>'Deliverd'];

      $userList = $this->admin_common_model->get_where('user_request',$arr_get);
...
?>
  ...
  <div class="content-wrapper">
    ...
    <form action="" method="POST" id="bulk_form">
      ...
      <span class="fs30 cdG">Deliverd Order </span>   
  ...
  <div class="tab-content">
    <div id="home" class="tab-pane fade in active">
      <div class="box-body table-responsive no-padding ">
        ...
        <thead>
            <tr>
                  ...
                  <th>Lessee Username</th> 
                  <th>Lessor Username</th>   
                  <th>Product Name</th> 
                  <th>Start Date</th>                 
                  <th>End Date </th>
                  <th>Amount</th>
      ...
      $Lessee_username = $this->admin_common_model->get_where('users',['id'=>$row['from_id']]);
      $Lessor_username = $this->admin_common_model->get_where('users',['id'=>$row['to_id']]);

      $product = $this->admin_common_model->get_where('product',['id'=>$row['product_id']]);
               ...
               ?>
                ...
                <tr>
                  ...
                  <div class="row">
                    ...
                    <sapn>  <?= $Lessee_username[0]['username']; ?>

                        <div>
                       <!-- <span><a href="<?=base_url('admin/view_page/addOrder/'.$row['id']);?>" class="cdG">Edit</a> | </span>  -->
                        <span><i class="fa fa-trash" aria-hidden="true"></i><a href="#" class="cdG" onclick="deleteUsers('<?=$row["id"];?>')"> Delete</a></span>
                        </div>
                    </sapn></div><!-- /row -->
                
                  </div><!-- col-sm-10 -->
                </td>
                <td><p><?= $Lessor_username[0]['username']; ?></p></td>
                <td><p><?= $product[0]['product_name']; ?></p></td>

                <td><p><?= $row['start_date']; ?></p></td>
                <td><p><?= $row['end_date']; ?></p></td>
                <td><p><?= $row['amount']; ?></p></td>
                <td><p><?= $row['address']; ?></p></td>
                <td><p><?= $row['status']; ?></p></td>
                <td><p><?= $row['payment_id']; ?></p></td>
                <td><p><?= $row['transaction_id']; ?></p></td>
                <td><p><?= $row['tracking_id']; ?></p></td>
                <td><p><?= $row['courier_name']; ?></p></td>
                <td><p><?= $row['estimate_date']; ?></p></td>
