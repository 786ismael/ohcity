...
<?php include 'include/header.php';
      ...
      $productIssue = $this->admin_common_model->get_all('product_issues');
...
?>
  ...
  <div class="content-wrapper">
    ...
    <form action="" method="POST" id="bulk_form">
      ...
      <span class="fs30 cdG">Product Issues </span><!-- <a href="<?=base_url('admin/view_page/AdNwProduct');?>"><button type="button" class=" btn  btn-info-add btn-md mt-10 ml10" > Add New</button></a>
-->
    ...
    <div id="home" class="tab-pane fade in active">
      <div class="box-body table-responsive no-padding ">
        ...
        <tbody>
           ...
           <?php $i=2; foreach($productIssue as $row){ 

                     $product      = $this->admin_common_model->get_where('product',['id'=>$row['product_id']]);
                     $issueTypes   = $this->admin_common_model->get_where('report_issue_categories',['id'=>$row['issue_id']]);
                     $users        = $this->admin_common_model->get_where('users',['id'=>$row['user_id']]);

               ?>

                <tr>
                   <td><?php echo ucfirst($product[0]['product_name']) ?></td>
                   <td><?php echo ucfirst($users[0]['first_name']) . ' ' . ucfirst($users[0]['last_name']) ?></td>
                   <td><?php echo $issueTypes[0]['issue_type'] ?></td>
